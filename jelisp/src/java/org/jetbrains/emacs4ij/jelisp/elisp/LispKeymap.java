 ...
 */
public abstract class LispKeymap implements LispObject {
    
    public abstract LispSymbol getKeyDefinition (Shortcut key);
    public abstract LispObject getKeyDefinition (LispStringOrVector key, @Nullable @Optional LispObject acceptDefault);

    public abstract LispKeymap getParent();
    public abstract void setParent(@Nullable LispKeymap parent);
    public abstract LispKeymap copy();
    ...
    public LispObject accessKeymap(LispObject idx, boolean tOk, boolean noInherit) {
        ...
        if (idx instanceof LispInteger && KeyBoardModifier.META.bitwiseAndNotZero((LispInteger) idx)) {
            ...
            LispObject metaMap = map.accessKeymap(meta_prefix_char, tOk, noInherit);
            if (metaMap instanceof LispList) {
                map = (LispKeymap) metaMap;
...
//            BuiltinsCore.error("attempt to define a key in a non-keymap");
        ...
        LispList insertion_point = (LispList) this;
    ...
    private void fixSubmapInheritance(LispObject event, LispObject submap) {
        ...
        LispObject parentEntry = !mapParent.equals(LispSymbol.ourNil)
                ? getKeymap(((LispKeymap) mapParent).accessKeymap(event, false, false))
                : null;
        ...
        if (! BuiltinsCore.eqs (parentEntry, submap)) {
            ...
            while (true) {
                ...
                if (isKeymap(tmp)) {
                    if (getKeymap(tmp).keymapMemberP(parentEntry)) // Fset_keymap_parent could create a cycle.
                        return;
                    submapParent = tmp;
                }
                else

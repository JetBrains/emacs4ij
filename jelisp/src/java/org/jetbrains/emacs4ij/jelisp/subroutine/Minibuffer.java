...
public abstract class Minibuffer {
  ...
  public static LispObject readFromMinibuffer (Environment environment, LispString prompt,
    ...
    if (read == null) {
      //todo
      GlobalEnvironment.echo("Parameter was not read", GlobalEnvironment.MessageType.ERROR);
      return null;
    }
  ...
  public static void goOn (InteractiveReader reader) {
    try {
      ...
      if (result != null) {
        GlobalEnvironment.echo(result.toString() + " (Minibuffer.goOn)", GlobalEnvironment.MessageType.OUTPUT);
      }
    } catch (LispException exc) {
      GlobalEnvironment.echo(exc.getMessage(), GlobalEnvironment.MessageType.ERROR);
    }

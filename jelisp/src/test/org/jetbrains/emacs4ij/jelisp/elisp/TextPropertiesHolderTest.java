...
public class TextPropertiesHolderTest {
  ...
  public void testAddProperties() {
    ...
    Assert.assertTrue(holder.getTextPropertiesHolder().actOnTextProperties(0, 11, LispList.list(new LispSymbol("one"), new LispInteger(1)),
        TextPropertiesHolder.Action.ADD));
    List<TextPropertiesHolder.TextPropertiesInterval> intervals = holder.getTextPropertiesHolder().getIntervals();
    Assert.assertEquals(1, intervals.size());
    Assert.assertTrue(intervals.get(0).getRange().equals(0, 11));
    Assert.assertEquals("#(\"hello world\" 0 11 (one 1))", holder.toString());

    Assert.assertTrue(holder.getTextPropertiesHolder().actOnTextProperties(4, 6, LispList.list(new LispSymbol("two"), new LispInteger(2)),
        TextPropertiesHolder.Action.ADD));
    ...
    Assert.assertTrue(holder.getTextPropertiesHolder().actOnTextProperties(4, 6,

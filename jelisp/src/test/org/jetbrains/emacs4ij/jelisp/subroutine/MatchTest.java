...
public class MatchTest extends JelispTestCase {
  ...
  public void testMatchBeginningOutOfRange() {
    try {
      ...
      Assert.assertEquals("'(args-out-of-range -10 0)", getCauseMsg(e));
      return;
    }
    Assert.fail();
  }
  ...
  public void testMatchStringOutOfRange() {
    try {
      ...
      Assert.assertEquals("'(args-out-of-range -1 0)", getCauseMsg(e));
      return;
    }
    Assert.fail();
  }
  ...
  public void setEmptyMatchDataToWrongInMiddleList() {
    try {
      ...
      Assert.assertEquals("'(wrong-type-argument interger-or-marker-p a)", getCauseMsg(e));
      return;
    }
    Assert.fail();
  ...
  public void setEmptyMatchDataToWrongList() {
    try {
      ...
      Assert.assertEquals("'(wrong-type-argument listp a)", getCauseMsg(e));
      LispObject data = evaluateString("(match-data)");
      Assert.assertEquals(LispList.list(new LispInteger(4), new LispInteger(9)), data);
      return;
    }
  ...
  public void testReplaceMatchInStringNilNil () {
    ...
    try {
      ...
      Assert.assertEquals("'(args-out-of-range -1 -1)", getCauseMsg(e));
      return;
    }
    Assert.fail();
  }
  ...
  public void testMatchStringOutOfStringRange () {
    ...
    try {
      ...
      Assert.assertEquals("'(args-out-of-range \"qwasqwas\" 10 2)", getCauseMsg(e));
...
//            Assert.assertEquals("'(args-out-of-range \"qwasqwas\" 10 2)", TestSetup.getCause(e));
      Assert.assertEquals("'(args-out-of-range 10 2)", getCauseMsg(e));
      return;
    }
    Assert.fail();
  }
  ...
  public void testReplaceMatchWithBackslashNonLiterally() {
    try {
      ...
      Assert.assertEquals("Invalid use of `\\\\' in replacement text", getCauseMsg(e));
      return;
    }
    Assert.fail();
  }
